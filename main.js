  ...
  if (serviceHeroH2) {
    ...
    if (!serviceHeroH2.querySelector('span.letter')) {
      ...
      html.split('<br>').forEach((line, lineIdx) => {
        if (lineIdx > 0) serviceHeroH2.appendChild(document.createElement('br'));
        line.replace(/<[^>]+>/g, '').split('').forEach((char, i) => {
          const span = document.createElement('span');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const statCards = document.querySelectorAll('.work-stat-card .work-stat-value');
  let statsAnimated = false;

  function animateCount(el, target, duration = 1500) {
    let start = 0;
    let startTime = null;
    target = parseFloat(target.toString().replace(/,/g, '').replace(/[^\d.]/g, ''));
    function updateCount(currentTime) {
      if (!startTime) startTime = currentTime;
      const progress = Math.min((currentTime - startTime) / duration, 1);
      const value = Math.floor(progress * (target - start) + start);
      el.textContent = value.toLocaleString() + (/\+/.test(el.dataset.suffix || '') ? '+' : '');
      if (progress < 1) {
